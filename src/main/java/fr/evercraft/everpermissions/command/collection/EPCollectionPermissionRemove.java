...
public class EPCollectionPermissionRemove extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPCollectionPermissionRemove(final EverPermissions plugin, final EPCollectionPermission parent) {
        ...
        this.pattern = Args.builder()
        		...
        		.arg((source, args) -> {
	        		try {
	        			EUserSubject subject = EPCommand.getSubject(source, this.plugin.getService(), args.getArg(0).orElse(""), args.getArg(1).orElse(""));
	        			String typeUser = EPCommand.getTypeWorld(source, this.plugin.getService().getUserSubjects(), args.getWorld().getName());
	        			return subject.getSubjectData().getPermissions(typeUser).keySet();
	        		} catch (EMessageException e) {
	        			return this.getAllGroups(args.getWorld().getName());
	        		}
        		});
    }
	
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_PERMISSION_REMOVE.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_PERMISSION_REMOVE_DESCRIPTION.getText();
	}
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> argsList) throws EMessageException {
		...
		return this.command(source, subject, permission, world.getName(), typeUser);
	}
	
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUserSubject subject, final String permission, 
			final String worldName, final String typeUser) {
		
		if (subject.getSubjectData().getPermissions(typeUser).get(permission) == null) {
			EPMessages.COLLECTION_PERMISSION_REMOVE_ERROR.sender()
		...
		return subject.getSubjectData().setPermission(typeUser, permission, Tristate.UNDEFINED)
			...
			.thenApply(result -> {
				...
				EPMessages.COLLECTION_PERMISSION_REMOVE.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
					.replace("{permission}", permission)
					.replace("{type}", typeUser)
				...
				this.plugin.getService().broadcastMessage(staff,
					EPMessages.COLLECTION_PERMISSION_REMOVE_BROADCAST.sender()
