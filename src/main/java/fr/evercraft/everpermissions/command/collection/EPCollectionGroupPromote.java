...
public class EPCollectionGroupPromote extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPCollectionGroupPromote(final EverPermissions plugin, final EPCollectionGroup parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_GROUP_PROMOTE.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_GROUP_PROMOTE_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUserSubject subject, final EGroupSubject group, final String worldName, final String typeUser) {
		...
		if (oldGroup.isPresent()) {
			...
			if (oldGroup.get().equals(group.asSubjectReference())) {
				EPMessages.COLLECTION_GROUP_PROMOTE_ERROR.sender()
			...
			if (oldGroup.isPresent() && oldGroup.get().resolve().join().isChildOf(contexts, group.asSubjectReference())) {
				EPMessages.COLLECTION_GROUP_PROMOTE_ERROR_DEMOTE.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
		...
		return subject.getSubjectData().setGroup(typeUser, group.asSubjectReference())
			...
			.thenApply(result -> {
				...
				EPMessages.COLLECTION_GROUP_PROMOTE.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
					.replace("{group}", groupName)
					.replace("{type}", typeUser)
				...
				this.plugin.getService().broadcastMessage(staff, 
					EPMessages.COLLECTION_GROUP_PROMOTE_BROADCAST.sender()
