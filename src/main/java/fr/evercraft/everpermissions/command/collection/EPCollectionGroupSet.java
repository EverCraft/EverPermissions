...
public class EPCollectionGroupSet extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPCollectionGroupSet(final EverPermissions plugin, final EPCollectionGroup parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_GROUP_SET.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_GROUP_SET_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUserSubject subject, final EGroupSubject group, 
		...
		if (oldGroup.isPresent() && oldGroup.get().equals(group.asSubjectReference())) {
			EPMessages.COLLECTION_GROUP_SET_ERROR.sender()
		...
		return subject.getSubjectData().setGroup(typeUser, group.asSubjectReference())
			...
			.thenApply(result -> {
				...
				EPMessages.COLLECTION_GROUP_SET.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
					.replace("{group}", groupName)
					.replace("{type}", typeUser)
				...
				this.plugin.getService().broadcastMessage(staff, 
					EPMessages.COLLECTION_GROUP_SET_BROADCAST.sender()
