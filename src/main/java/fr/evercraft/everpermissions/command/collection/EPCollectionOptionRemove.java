...
public class EPCollectionOptionRemove extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPCollectionOptionRemove(final EverPermissions plugin, final EPCollectionOption parent) {
        ...
        this.pattern = Args.builder()
        		...
        		.arg((source, args) -> {
	        		try {
	        			...
	        			return subject.getSubjectData().getOptions(typeUser).keySet();
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_OPTION_REMOVE.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_OPTION_REMOVE_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUserSubject subject, final String option, final String worldName, final String typeUser) {
		if (subject.getSubjectData().getOptions(typeUser).get(option) == null) {
			EPMessages.COLLECTION_OPTION_REMOVE_ERROR.sender()
		...
		return subject.getSubjectData().setOption(typeUser, option, null)
			...
			.thenApply(result -> {
				...
				EPMessages.COLLECTION_OPTION_REMOVE.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
					.replace("{option}", option)
					.replace("{type}", typeUser)
