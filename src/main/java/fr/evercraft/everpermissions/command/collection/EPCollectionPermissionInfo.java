...
public class EPCollectionPermissionInfo extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	private final EPCollectionPermission parent;
	
	public EPCollectionPermissionInfo(final EverPermissions plugin, final EPCollectionPermission parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_PERMISSION_INFO.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_PERMISSION_INFO_DESCRIPTION.getText();
	}
	...
	private void command(final CommandSource player, final EUserSubject subject, final String worldName, final String typeUser) {
		...
		this.parent.getParent().getInfo().addPermissions(list, subject, worldName, typeUser);
		this.parent.getParent().getInfo().addPermissionsTransient(list, subject, worldName, typeUser);
		
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.COLLECTION_PERMISSION_INFO_TITLE.getFormat().toText(
