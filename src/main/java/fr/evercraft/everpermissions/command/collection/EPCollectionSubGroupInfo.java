...
public class EPCollectionSubGroupInfo extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	private final EPCollectionSubGroup parent;
	
	public EPCollectionSubGroupInfo(final EverPermissions plugin, final EPCollectionSubGroup parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_SUBGROUP_INFO.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_SUBGROUP_INFO_DESCRIPTION.getText();
	}
	...
	private void command(final CommandSource player, final EUserSubject subject, final String worldName, final String typeUser) {
		...
		this.parent.getParent().getInfo().addSubGroups(list, subject, worldName, typeUser);
		this.parent.getParent().getInfo().addSubGroupsTransient(list, subject, worldName, typeUser);
		
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.COLLECTION_SUBGROUP_INFO_TITLE.getFormat().toText(
