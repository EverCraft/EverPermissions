...
public class EPCollectionSubGroupRemove extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPCollectionSubGroupRemove(final EverPermissions plugin, final EPCollectionSubGroup parent) {
        ...
        this.pattern = Args.builder()
        		...
        		.arg((source, args) -> {
	        		try {
	        			...
	        			return subject.getSubjectData().getSubGroup(typeUser).stream()
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.COLLECTION_SUBGROUP_REMOVE.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.COLLECTION_SUBGROUP_REMOVE_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUserSubject subject, final EGroupSubject group, 
		...
		if (!subject.getSubjectData().getSubGroup(typeUser).contains(group.asSubjectReference())) {
			EPMessages.COLLECTION_SUBGROUP_REMOVE_ERROR.sender()
		...
		return subject.getSubjectData().removeParent(typeUser, group.asSubjectReference())
			...
			.thenApply(result -> {
				...
				EPMessages.COLLECTION_SUBGROUP_REMOVE.sender()
					.replace("{subject}", subject.getIdentifier())
					.replace("{collection}", subject.getCollectionIdentifier())
					.replace("{group}", groupName)
					.replace("{type}", typeUser)
				...
				this.plugin.getService().broadcastMessage(staff,
					EPMessages.COLLECTION_SUBGROUP_REMOVE_BROADCAST.sender()
