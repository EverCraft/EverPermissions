...
public class EPUserPermissionInfo extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	private final EPUserPermission parent;
	
	public EPUserPermissionInfo(final EverPermissions plugin, final EPUserPermission parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_PERMISSION_INFO.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_PERMISSION_INFO_DESCRIPTION.getText();
	}
	...
	private void command(final CommandSource player, final EUser user, final EPUserSubject subject, final String worldName, final String typeUser) {
		...
		this.parent.getParent().getInfo().addPermissions(list, user, subject, worldName, typeUser);
		this.parent.getParent().getInfo().addPermissionsTransient(list, user, subject, worldName, typeUser);
		
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.USER_PERMISSION_INFO_TITLE.getFormat().toText(
					"{player}", user.getName(),
					"{type}", typeUser)
				.toBuilder()
