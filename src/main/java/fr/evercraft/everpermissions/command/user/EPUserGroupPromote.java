...
public class EPUserGroupPromote extends ESubCommand<EverPermissions> {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_GROUP_PROMOTE.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_GROUP_PROMOTE_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final EUserSubject subject, final EGroupSubject group, 
		...
		if (oldGroup.isPresent()) {
			...
			if (oldGroup.get().equals(group.asSubjectReference())) {
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_GROUP_PROMOTE_ERROR_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(staff);
				} else {
					EPMessages.USER_GROUP_PROMOTE_ERROR_STAFF.sender()
						.replace("{player}", user.getName())
			...
			if (oldGroup.isPresent() && oldGroup.get().resolve().join().isChildOf(contexts, group.asSubjectReference())) {
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_GROUP_PROMOTE_ERROR_DEMOTE_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
					...
					EPMessages.USER_GROUP_PROMOTE_ERROR_DEMOTE_STAFF.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{parent}", oldGroup.get().getSubjectIdentifier())
						.replace("{type}", typeUser)
		...
		return subject.getSubjectData().setGroup(typeUser, group.asSubjectReference())
			...
			.thenApply(result -> {
				...
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_GROUP_PROMOTE_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(staff);
					...
					this.plugin.getService().broadcastMessage(staff, 
						EPMessages.USER_GROUP_PROMOTE_BROADCAST_EQUALS.sender()
							.replace("{staff}", staff.getName())
							.replace("{player}", user.getName())
							.replace("{group}", groupName)
							.replace("{type}", typeUser));
				} else {
					EPMessages.USER_GROUP_PROMOTE_STAFF.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(staff);
					
					EPMessages.USER_GROUP_PROMOTE_PLAYER.sender()
						.replace("{staff}", staff.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(user);
					...
					this.plugin.getService().broadcastMessage(staff, user.getUniqueId(), 
						EPMessages.USER_GROUP_PROMOTE_BROADCAST_PLAYER.sender()
							.replace("{staff}", staff.getName())
							.replace("{player}", user.getName())
							.replace("{group}", groupName)
							.replace("{type}", typeUser));
