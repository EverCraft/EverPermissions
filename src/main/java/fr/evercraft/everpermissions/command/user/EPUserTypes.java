...
public class EPUserTypes extends ESubCommand<EverPermissions> {
	
	public EPUserTypes(final EverPermissions plugin, final EPUser parent) {
        super(plugin, parent, "types");
	}
	
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_TYPES.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_TYPES_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource player, final EGroupCollection collection) throws EMessageException {
		...
		types.forEach((world, type) -> {
			list.add(EPMessages.USER_TYPES_LINE.getFormat()
					.toText("{world}", EChat.getButtomCopy(world),
							"{type}", EChat.getButtomCopy(type)));
		});
		
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.USER_TYPES_TITLE.getText()
					.toBuilder()
