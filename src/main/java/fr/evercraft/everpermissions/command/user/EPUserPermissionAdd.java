...
public class EPUserPermissionAdd extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPUserPermissionAdd(final EverPermissions plugin, final EPUserPermission parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_PERMISSION_ADD.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_PERMISSION_ADD_DESCRIPTION.getText();
	}
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> argsList) throws EMessageException {
		...
		boolean value = args.getArg(2, Args.BOOLEAN).orElse(false);
		...
		return UtilsCompletableFuture.anyOf(args.getArg(0, Args.USERS).orElse(Arrays.asList()).stream()
				.map(user -> this.command(source, user, permission, value, world.getName(), typeUser))
				.collect(Collectors.toList()));
	}
	
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final String permission, final boolean value, final String worldName, final String typeUser) {
		return this.plugin.getService().getUserSubjects().load(user.getIdentifier())
			...
			.thenCompose(subject -> {
				...
				return this.command(staff, user, subject, permission, value, worldName, typeUser);
			});
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final EUserSubject subject, final String permission, 
		...
		if (oldValue != null) {
			...
			if (value) {
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_PERMISSION_ADD_TRUE_ERROR_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{permission}", permission)
						.replace("{type}", typeUser)
						.sendTo(staff);
				} else {
					EPMessages.USER_PERMISSION_ADD_TRUE_ERROR_STAFF.sender()
						.replace("{player}", user.getName())
						.replace("{permission}", permission)
						.replace("{type}", typeUser)
						.sendTo(staff);
			...
			// Permission : False
				...
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_PERMISSION_ADD_FALSE_ERROR_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{permission}", permission)
						.replace("{type}", typeUser)
						.sendTo(staff);
				} else {
					EPMessages.USER_PERMISSION_ADD_FALSE_ERROR_STAFF.sender()
						.replace("{player}", user.getName())
						.replace("{permission}", permission)
						.replace("{type}", typeUser)
						.sendTo(staff);
		...
		return subject.getSubjectData().setPermission(typeUser, permission, Tristate.fromBoolean(value))
			...
			.thenApply(result -> {
				...
				if (value) {
					if (staff.getIdentifier().equals(user.getIdentifier())) {
						EPMessages.USER_PERMISSION_ADD_TRUE_EQUALS.sender()
							.replace("{player}", user.getName())
							.replace("{permission}", permission)
							.replace("{type}", typeUser)
							.sendTo(staff);
						...
						this.plugin.getService().broadcastMessage(staff,
							EPMessages.USER_PERMISSION_ADD_TRUE_BROADCAST_EQUALS.sender()
								.replace("{staff}", staff.getName())
								.replace("{player}", user.getName())
								.replace("{permission}", permission)
								.replace("{type}", typeUser));
					} else {
						EPMessages.USER_PERMISSION_ADD_TRUE_STAFF.sender()
							.replace("{player}", user.getName())
							.replace("{permission}", permission)
							.replace("{type}", typeUser)
							.sendTo(staff);
						...
						this.plugin.getService().broadcastMessage(staff,
							EPMessages.USER_PERMISSION_ADD_TRUE_BROADCAST_PLAYER.sender()
								.replace("{staff}", staff.getName())
								.replace("{player}", user.getName())
								.replace("{permission}", permission)
								.replace("{type}", typeUser));
				...
				// Permission : False
					...
					if (staff.getIdentifier().equals(user.getIdentifier())) {
						EPMessages.USER_PERMISSION_ADD_FALSE_EQUALS.sender()
							.replace("{player}", user.getName())
							.replace("{permission}", permission)
							.replace("{type}", typeUser)
							.sendTo(staff);
						...
						this.plugin.getService().broadcastMessage(staff,
							EPMessages.USER_PERMISSION_ADD_FALSE_BROADCAST_EQUALS.sender()
								.replace("{staff}", staff.getName())
								.replace("{player}", user.getName())
								.replace("{permission}", permission)
								.replace("{type}", typeUser));
					} else {
						EPMessages.USER_PERMISSION_ADD_FALSE_STAFF.sender()
							.replace("{player}", user.getName())
							.replace("{permission}", permission)
							.replace("{type}", typeUser)
							.sendTo(staff);
						...
						this.plugin.getService().broadcastMessage(staff,
							EPMessages.USER_PERMISSION_ADD_FALSE_BROADCAST_PLAYER.sender()
								.replace("{staff}", staff.getName())
								.replace("{player}", user.getName())
								.replace("{permission}", permission)
								.replace("{type}", typeUser));
