...
public class EPUserSubGroupAdd extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	
	public EPUserSubGroupAdd(final EverPermissions plugin, final EPUserSubGroup parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_SUBGROUP_ADD.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_SUBGROUP_ADD_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final EUserSubject subject, final EGroupSubject group, 
		...
		if (subject.getSubjectData().getSubGroup(typeUser).contains(group.asSubjectReference())) {
			if (staff.getIdentifier().equals(user.getIdentifier())) {
				EPMessages.USER_SUBGROUP_ADD_ERROR_EQUALS.sender()
					.replace("{player}", user.getName())
					.replace("{group}", groupName)
					.replace("{type}", typeUser)
					.sendTo(staff);
			} else {
				EPMessages.USER_SUBGROUP_ADD_ERROR_STAFF.sender()
					.replace("{player}", user.getName())
					.replace("{group}", groupName)
					.replace("{type}", typeUser)
					.sendTo(staff);
		...
		return subject.getSubjectData().addParent(typeUser, group.asSubjectReference())
			...
			.thenApply(result -> {
				...
				if (staff.getIdentifier().equals(user.getIdentifier())) {
					EPMessages.USER_SUBGROUP_ADD_EQUALS.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(staff);
					...
					this.plugin.getService().broadcastMessage(staff,
						EPMessages.USER_SUBGROUP_ADD_BROADCAST_EQUALS.sender()
							.replace("{staff}", staff.getName())
							.replace("{player}", user.getName())
							.replace("{group}", groupName)
							.replace("{type}", typeUser));
				} else {
					EPMessages.USER_SUBGROUP_ADD_STAFF.sender()
						.replace("{player}", user.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(staff);
					
					EPMessages.USER_SUBGROUP_ADD_PLAYER.sender()
						.replace("{staff}", staff.getName())
						.replace("{group}", groupName)
						.replace("{type}", typeUser)
						.sendTo(user);
					...
					this.plugin.getService().broadcastMessage(staff, user.getUniqueId(), 
						EPMessages.USER_SUBGROUP_ADD_BROADCAST_PLAYER.sender()
							.replace("{staff}", staff.getName())
							.replace("{player}", user.getName())
							.replace("{group}", groupName)
							.replace("{type}", typeUser));
