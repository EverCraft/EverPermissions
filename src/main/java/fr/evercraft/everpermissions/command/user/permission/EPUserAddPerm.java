...
public class EPUserAddPerm extends ECommand<EverPermissions> {
	...
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final String permission, final String value_name, final String world_name) {
		return this.plugin.getService().getUserSubjects().load(user.getIdentifier())
			...
			.thenCompose(subject -> {
				...
				return this.command(staff, user, subject, permission, value_name, world_name);
			});
	}
	
	private CompletableFuture<Boolean> command(final CommandSource staff, final EUser user, final EUserSubject subject, final String permission, final String value_name, final String world_name) {
		...
		Boolean oldValue = subject.getSubjectData().getPermissions(type_user.get()).get(permission);
		if (oldValue != null) {
			// Permission : True
			if (value.get()) {
		...
		return subject.getSubjectData().setPermission(type_user.get(), permission, Tristate.fromBoolean(value.get()))
