...
public class EPUserOptionInfo extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	private final EPUserOption parent;
	
	public EPUserOptionInfo(final EverPermissions plugin, final EPUserOption parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.USER_OPTION_INFO.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.USER_OPTION_INFO_DESCRIPTION.getText();
	}
	...
	private void command(final CommandSource player, final EUser user, final EPUserSubject subject, final String worldName, final String typeUser) {
		...
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.USER_OPTION_INFO_TITLE.getFormat().toText(
					"{player}", user.getName(),
					"{type}", typeUser)
				.toBuilder()
