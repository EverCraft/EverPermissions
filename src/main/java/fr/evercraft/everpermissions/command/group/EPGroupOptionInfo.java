...
public class EPGroupOptionInfo extends ESubCommand<EverPermissions> {
	
	private final Args.Builder pattern;
	private final EPGroupOption parent;
	
	public EPGroupOptionInfo(final EverPermissions plugin, final EPGroupOption parent) {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(EPPermissions.GROUP_OPTION_INFO.get());
	}

	public Text description(final CommandSource source) {
		return EPMessages.GROUP_OPTION_INFO_DESCRIPTION.getText();
	}
	...
	private CompletableFuture<Boolean> command(final CommandSource player, final String groupName, final String worldName) throws EMessageException {
		...
		this.parent.getParent().getInfo().addOptions(list, group, worldName, typeGroup);
		this.parent.getParent().getInfo().addOptionsTransient(list, group, worldName, typeGroup);
		
		this.plugin.getEverAPI().getManagerService().getEPagination().sendTo(
				EPMessages.GROUP_OPTION_INFO_TITLE.getFormat().toText(
					"{group}", group.getName(),
					"{type}", typeGroup)
				.toBuilder()
