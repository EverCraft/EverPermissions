...
public class EPUserDelSubGroup extends ECommand<EverPermissions> {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(this.plugin.getPermissions().get("USER_DEL_SUBGROUP"));
	}

	public Text description(final CommandSource source) {
		return this.plugin.getMessages().getText("USER_DEL_SUBGROUP_DESCRIPTION");
	}

	public Text help(final CommandSource source) {
		return Text.builder("/permudelsub <" + this.plugin.getEverAPI().getMessages().getArg("player") + "> "
									   + "<" + this.plugin.getEverAPI().getMessages().getArg("subgroup") + "> "
		...
		if(type_user.isPresent() && type_group.isPresent()) {
			...
			if(group != null && group.hasWorld(type_group.get())) {
				...
				if(subject != null) {
					...
					if(subject.getSubjectData().removeSubParent(EContextCalculator.getContextWorld(type_user.get()), group)) {
						if(staff.equals(user)) {
							staff.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("USER_DEL_SUBGROUP_EQUALS")
									.replaceAll("<player>", user.getName())
									.replaceAll("<group>", group.getIdentifier())
									.replaceAll("<type>", type_user.get())));
						} else {
							staff.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("USER_DEL_SUBGROUP_STAFF")
									.replaceAll("<player>", user.getName())
									.replaceAll("<group>", group.getIdentifier())
									.replaceAll("<type>", type_user.get())));
							...
							if(player.isPresent()) {
								player.get().sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("USER_DEL_SUBGROUP_PLAYER")
					...
					// Le sous-groupe n'a pas été supprimé
						...
						if(staff.equals(user)) {
							staff.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("USER_DEL_SUBGROUP_ERROR_EQUALS")
									.replaceAll("<player>", user.getName())
									.replaceAll("<group>", group.getIdentifier())
									.replaceAll("<type>", type_user.get())));
						} else {
							staff.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("USER_DEL_SUBGROUP_ERROR_STAFF")
									.replaceAll("<player>", user.getName())
									.replaceAll("<group>", group.getIdentifier())
									.replaceAll("<type>", type_user.get())));
						}
