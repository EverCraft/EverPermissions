...
public class EPUserCheckOption extends ECommand<EverPermissions> {
	...
	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(this.plugin.getPermissions().get("USER_CHECK_OPTION"));
	}

	public Text description(final CommandSource source) {
		return this.plugin.getMessages().getText("USER_CHECK_OPTION_DESCRIPTION");
	}

	public Text help(final CommandSource source) {
		return Text.builder("/permuchecko <" + this.plugin.getEverAPI().getMessages().getArg("player") + "> "
				...
				if(name.isPresent()) {
					...
					if(staff.equals(user)) {
						staff.sendMessage(ETextBuilder.toBuilder(this.plugin.getMessages().getText("PREFIX"))
							.append(this.plugin.getMessages().getMessage("USER_CHECK_OPTION_DEFINED_EQUALS")
					...
					// La source et le joueur ne sont pas identique
						...
						staff.sendMessage(ETextBuilder.toBuilder(this.plugin.getMessages().getText("PREFIX"))
								.append(this.plugin.getMessages().getMessage("USER_CHECK_OPTION_DEFINED_STAFF")
									.replaceAll("<player>", user.getName())
									.replaceAll("<option>", type)
									.replaceAll("<type>", type_user.get()))
								.replace("<value>", Text.builder(name.get())
