...
public class EPGroupDelOption extends ECommand<EverPermissions> {
	...
	public EPGroupDelOption(final EverPermissions plugin) {
        super(plugin, "permgdelo", "mangdelv");
    }

	public boolean testPermission(final CommandSource source) {
		return source.hasPermission(this.plugin.getPermissions().get("USER_DEL_OPTION"));
	}

	public Text description(final CommandSource source) {
		return this.plugin.getMessages().getText("USER_DEL_OPTION_DESCRIPTION");
	}

	public Text help(final CommandSource source) {
		return Text.builder("/permgdelo <" + this.plugin.getEverAPI().getMessages().getArg("group") + "> "
									 + "<" + this.plugin.getEverAPI().getMessages().getArg("option") + "> "
									 + "[" + this.plugin.getEverAPI().getMessages().getArg("world") + "]")
					.onClick(TextActions.suggestCommand("/permgdelo "))
	...
	private boolean command(final CommandSource player, final String group_name, final String option, final String world_name) {
		...
		if(type_group.isPresent()) {
			...
			if(group != null && group.hasWorld(type_group.get())) {
				...
				if(group.getSubjectData().setOption(EContextCalculator.getContextWorld(type_group.get()), option, null)) {
					player.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("GROUP_DEL_OPTION_STAFF")
				...
				// L'option n'a pas été supprimé
					...
					player.sendMessage(UtilsChat.of(this.plugin.getMessages().getMessage("PREFIX") + this.plugin.getMessages().getMessage("GROUP_DEL_OPTION_ERROR")
							.replaceAll("<group>", group.getIdentifier())
							.replaceAll("<option>", option)
							.replaceAll("<type>", type_group.get())));
				}
